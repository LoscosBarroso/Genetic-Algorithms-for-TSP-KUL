...
resettxt = uicontrol(ph,'Style','text','String','Reset (Early Stop)','Position',[220 310 130 20]);
reset = uicontrol(ph,'Style','popupmenu','String',{'off','on'},'Value',1,'Position',[340 310 50 20],'Callback',@reset_Callback);
...
crossovertxt = uicontrol(ph,'Style','text','String','Crossover','Position',[0 50 130 20]);
crossover = uicontrol(ph,'Style','popupmenu', 'String',{'xalt_edges'}, 'Value',1,'Position',[130 50 130 20],'Callback',@crossover_Callback);
...
set(fh,'Visible','on');
    ...
    function runbutton_Callback(hObject,eventdata)
        ...
        run_ga(x, y, NIND, MAXGEN, NVAR, ELITIST, STOP_PERCENTAGE, PR_CROSS, PR_MUT, CROSSOVER, LOCALLOOP, EARLY, STALL_PERCENTAGE, RESET, ah1, ah2, ah3);
        end_run();
    end
